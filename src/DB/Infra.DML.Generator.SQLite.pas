unit Infra.DML.Generator.SQLite;
interface
uses
  Classes, SysUtils,
...
procedure TDMLGeneratorSQLite.GenerateSQLPaginating(const APage, ARowsPerPage: Integer; const ASQL: TStrings);
var
  LSQLCommand: string;
...
function TDMLGeneratorSQLite.GetColumnNameTotalPages: string;
begin
  Result := ColumnNameTotalPages;
end;
initialization
TDBDriverRegister.RegisterDriver(TDBDriver.SQLite, {$IF DEFINED(INFRA_ORMBR)}TDriverName.dnSQLite, {$IFEND}TDMLGeneratorSQLite.Create);
end.
